#
# trade/process/process_entry_request.py
#

from datetime import datetime

from core.logger import Log

from trade.trade_enums import SideType

from market.order_enums import (
    OrderType,
    OrderAction,
    OrderState,
)

from trade.process.process_order_base import (
    ProcessOrderBase,
    ORDER_SUBMIT_TIMEOUT_SEC,
)

from core.exception import (
    InternalError,
    OrderSubmitTimeoutError,
)


class ProcessEntryRequest(ProcessOrderBase):

    def __init__(self, context, market):
        super().__init__(context, market)

        Log.create("ProcessEntryRequest")

    def process(self, trade):

        Log.flow(f"(#{trade.id}) ProcessEntryRequest:process")

        order = trade.entry_order

        if order is None:
            order = self.create_entry_order(trade)

            order.change_state(OrderState.CREATED)

            return False

        else:
            match order.state:
                case OrderState.CREATED:
                    result = self.request_order(trade, order)
                    if not result:
                        return False

                    # 発注受付待ち開始時刻
                    # Orderのタイムアウトチェックの為、
                    # SUBMITTEDへ移行する直前に時刻を保存する。
                    order.submitted_at = datetime.now()

                    order.change_state(OrderState.SUBMITTED)

                case OrderState.SUBMITTED:

                    # 発注受付待ちタイムアウト
                    if order.submitted_at is None:
                        raise InternalError(
                            message=f"(#{trade.id}) @({order.id}) Order submitted_at is None at ProcessEntryRequest",
                            code="ORDER_SUBMIT_TIMESTAMP_MISSING",
                        )

                    elapsed = (datetime.now() - order.submitted_at).total_seconds()

                    if elapsed >= ORDER_SUBMIT_TIMEOUT_SEC:
                        raise OrderSubmitTimeoutError(
                            message=f"(#{trade.id}) @({order.id}) Order submit timeout at ProcessEntryRequest: elapsed={elapsed:.1f}s",
                            code="ORDER_SUBMIT_TIMEOUT",
                        )

                    order.order_no = self.get_order_no(trade, order)
                    if order.order_no is None:
                        Log.warn(f"(#{trade.id}) @({order.id}) GET ORDER NO NONE")
                        return False

                    Log.event(f"(#{trade.id}) @({order.id}) GET ORDER NO SUCCESS order_no={order.order_no}")
                    order.change_state(OrderState.REQUESTED)

                    # True の後は呼ばれない
                    return True

                case _:
                    raise Exception(f"(#{trade.id}) @({order.id}) UNKNOWN ORDER STATE {order.state}")

        return False


    #
    # Order生成
    #
    def create_entry_order(self, trade):

        if trade.param.side == SideType.LONG:
            order_action = OrderAction.BUY

        elif trade.param.side == SideType.SHORT:
            order_action = OrderAction.SELL

        else:
            raise InternalError(message=f"UNKNOWN SIDE {trade.param.side}", code="UNKNOWN_SIDE")

        # 成行注文
        order = self.create_order(
            trade,
            order_action,
            # 成行(OrderType.MARKET)だが、
            # DEBUGでは反転確定時の価格を約定価格として使用する。
            trade.runtime.entry_previous_price,
            OrderType.MARKET,
        )

        trade.entry_order = order

        return order