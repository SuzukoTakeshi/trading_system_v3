...
from core.exception import OrderDuplicateError
...
class ProcessOrderBase(ProcessBase):
    ...
    def create_order(self, trade, order_action, price, order_type=OrderType.MARKET, order_role="entry"):
        ...
        Log.event(
            f"(#{trade.id}) (@{order.id}) CREATE ORDER "
            f"symbol={trade.param.symbol} action={order_action.value}"
        )
    ...
    def request_order(self, trade, order):
        ...
        if result:
            message = f"(@{order.id}) ORDER REQUEST SUCCESS symbol={order.symbol}"
            Log.event(f"(#{trade.id}) {message}")
            trade.add_timeline(type="ORDER", message=message)
            ...
            message = f"(@{order.id}) ORDER REQUEST FAILED symbol={order.symbol} error={result_code.value}"
            Log.event(f"(#{trade.id}) {message}")
            trade.add_timeline(type="ERROR", message=message)
    ...
    def get_order_no(self, trade, order):
        ...
        # 楽天RSSの発注ID
        #   Order IDはシステム内部のID。
        #   楽天RSSでは、Order ID × 10 + 発注ID枝番を発注IDとして使用する。
        rss_order_id = (order.id * 10 + order.order_id_sub_no)

        order.order_id_sheet_data = self.market.get_order_id_data(rss_order_id)
        ...
        result, order_result = self.market.get_order_no(rss_order_id)
        if result is None:
            if order_result:
                raise OrderResultError(
                    message=f"(#{trade.id}) @({order.id}) get_order_no: order_result={order_result}",
                    code="ORDER_GET_ORDER_NO_ERROR",
                )
            return None
