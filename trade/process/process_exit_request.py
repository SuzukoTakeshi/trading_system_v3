#
# trade/process/process_exit_request.py
#
...
class ProcessExitRequest(ProcessOrderBase):

    def __init__(self, context, market):
        super().__init__(context, market)

        Log.create("ProcessExitRequest")


    def process(self, trade):

        Log.flow(f"(#{trade.id}) ProcessExitRequest:process")
            ...
            match order.state:
                ...
                case OrderState.SUBMITTED:
                    ...
                    if order.submitted_at is None:
                        raise InternalError(
                            message=f"@({order.id}) Order submitted_at is None at ProcessExitRequest",
                    ...
                    if elapsed >= ORDER_SUBMIT_TIMEOUT_SEC:
                        raise OrderSubmitTimeoutError(
                            message=f"@({order.id}) Order submit timeout at ProcessExitRequest: elapsed={elapsed:.1f}s",
    ...
    def create_exit_order(self, trade):
        ...
        order = self.create_order(
            ...
            trade.runtime.exit_decision_price,
