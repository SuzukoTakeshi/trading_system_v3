...
# =RssOrderIDList($A$2:$F$2) => 配信中					
# 発注ID	関数名	発注日	発注時刻	注文番号	発注結果
# 999	国内株式 信用返済注文(VBA)	2026/09/08	12:18:55	-	エラー[該当する建玉情報がありません。]
# 106	国内株式 信用返済注文(VBA)	2026/09/08	16:13:58	-	エラー[現在の時間帯は、国内株式の注文を受付していません。17:15以降に再度注文してください。]
# 105	国内株式 信用返済注文(VBA)	2026/09/08	17:50:11	53059757	発注済み
# 104	国内株式 取消注文(VBA)	2026/09/08	17:55:30	53059757	発注済み
# --------	--------	--------	--------	--------	--------

# =RssOrderList($A$2:$AD$2, 0, 0, "", "A", 0, 0, 0, 0, 0) => 配信中																													
# 注文番号	受付No	通常注文状況	逆指値注文状況	アルゴ注文状況	銘柄コード	銘柄名称    口座区分	市場名称	信用区分	弁済期限	発注/受注日時	売買	取引	執行条件	注文期限	注文数量	約定数量	注文単価	注文区分	逆指値条件	セット注文	セット注文条件	税区分	注文失効日時	注文失効理由	入力経路	アルゴ注文条件	価格判定時刻	価格判定情報/対象外理由
# 53059757	#6592	執行待ち	        -		                5803       フジクラ    特定        東証(SOR)	制度	6ヶ月	2026/09/08 17:50:11	売埋	信用返済	本日中	20260909	100	0	成行	通常注文	-	-	-	源徴あり	-		Market Speed		2026/09/08 17:50:11	SOR対象外 (SORサービス時間外)
#  ↓																													
# 53059757	#6592	取消済（出来無）    -	                	5803        フジクラ    特定	   東証(SOR)	制度	6ヶ月	2026/09/08 17:50:11	売埋	信用返済	本日中	20260909	100	0	成行	通常注文	-	-	-	源徴あり	-		Market Speed		2026/09/08 17:50:11	SOR対象外 (SORサービス時間外)
